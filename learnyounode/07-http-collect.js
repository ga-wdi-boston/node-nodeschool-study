...
collect.pipe(bl(function (err, data) {
 if (err)
...
The solution about does not work, and I'm not sure why, plan to get some help with it.*/

var bl = require('bl');

require('http').get(process.argv[2], function (collect) {
  collect.pipe(require('bl')(function (err, data) {
    if (err)
      return console.error(err);
    data = data.toString();
    console.log(data.length);
